...
function evalInstr(instr: bril.Instruction, state: State): Action {
    ...
    switch (instr.op) {
        ...
        case "print": {
            ...
            // TODO: Modified to produce traces
            // console.log(...values);
            return NEXT;
        }
...
function evalFunc(func: bril.Function, state: State): Value | null {
    for (let i = 0; i < func.instrs.length; ++i) {
        ...
        // Keep track of number of times the line has been executed
...
function speculate(prog: bril.Program) {
    // let trace: bril.Instruction[] = [];
    // let tracing = false;
    // const threshold = 3;
    // Build a trace based on a heuristic: the sub-path is "hot"
    // for (let [instr, count] of instrCounts) {
    //     // Begin tracing condition
    //     if (count >= threshold) {
    //         tracing = true;
    //     }
    //     // Quit tracing condition
    //     else if (count < threshold && tracing) {
    //         break;
    //     }
    //     // Continue tracing, build clone of instruction
    //     if (tracing) {
    //         let instrClone = JSON.parse(JSON.stringify(instr));
    //         trace.push(instrClone);
    //     }
    // }
    // console.log(trace);
    // for (let instr of trace) {
    //     // TODO: clone each instruction and place into speculate block.
    //     //  Put the speculate block immediately before the original first insn.
    //     //  Remove control flow, change breaks to guards
    //
    //
    // }
