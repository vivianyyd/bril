...
export class GarbageCollector {
  ...
  sweep(heap: Heap<Value>, colors: Map<number, Color>) {
    for (let base of heap.storage.keys()) {
      if (!colors.has(base)) {
        heap.free(new Key(base, 0))
      }
    }
  }
  ...
  mark(stack: Array<Env>, heap: Heap<Value>): Map<number, Color> {
    ...
    for (const env of stack) {
      for (let [_, value] of env) {
        if (value.hasOwnProperty("loc")) {
          this.dfs(heap, value.loc, colors);
        }
      }
    }
    return colors;
  ...
  collectGarbage(stack: Array<Env>, heap: Heap<Value>) {
    this.count++;
    if (this.count == 5) {
 ...
 */
function evalCall(instr: bril.Operation, state: State, gc: GarbageCollector, stack: Array<Env>): Action {
  ...
  let retVal = evalFunc(func, newState, gc, stack);
 ...
 */
function evalInstr(instr: bril.Instruction, state: State, gc: GarbageCollector, stack: Array<Env>): Action {
  ...
  gc.collectGarbage(stack, state.heap);
  ...
  switch (instr.op) {
    ...
    case "call": {
      return evalCall(instr, state, gc, stack);
    }
...
function evalFunc(func: bril.Function, state: State, gc: GarbageCollector, stack: Array<Env>): Value | null {
  stack.push(state.env);

  for (let i = 0; i < func.instrs.length; ++i) {
    let line = func.instrs[i];
    if ('op' in line) {
      // Run an instruction.
      let action = evalInstr(line, state, gc, stack);
